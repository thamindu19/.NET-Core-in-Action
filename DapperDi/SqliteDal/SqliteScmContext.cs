...
namespace SqliteDal {
  public class SqliteScmContext : IScmContext {
    ...
    public SqliteScmContext(SqliteConnection conn) {
      connection = conn;
      // Open can be safely called multiple times
      conn.Open();
    ...
    public PartCommand[] GetPartCommands() {
      // ToArray reads all the values from the IEnumerable into an array
    ...
    public void DeletePartCommand(int id, DbTransaction transaction) {
      // Uses an anonymous type for the Id parameter
    ...
    public IEnumerable<Order> GetOrders() {
      var orders = connection.Query<Order>(
        "SELECT * FROM [Order]");
      // foreach will get all Order objects in the IEnumerable, just like ToArray
